...
class MainWindow(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        add_files_action.setStatusTip(
            "Bilgisayarınızdan dosya seçerek mevzuat belgesi ekleyin"
        )
        ...
        scan_action.setStatusTip(
            "Raw klasöründeki işlenmemiş dosyaları sistem otomatik tarar"
        )
        ...
        rebuild_index_action.setStatusTip(
            "Tüm belgeler için arama indeksini yeniden oluşturur"
        )
        ...
        vacuum_action.setStatusTip(
            "Veritabanını optimize eder ve gereksiz alanları temizler"
        )
    ...
    def create_left_panel(self) -> QWidget:
        ...
        self.recent_searches_list.itemDoubleClicked.connect(
            self.on_recent_search_selected
        )
    ...
    def create_middle_panel(self) -> QWidget:
        ...
        # Sonuçlar için splitter
        splitter = QSplitter(Qt.Vertical)
        
        # Üst kısım: Arama sonuçları ve önizleme
        self.search_results_widget = SearchResultsWidget(self.db)
        self.search_results_widget.document_opened.connect(self.on_preview_document_selected)
        
        # Eski sonuç widget'ını da ekleyelim (geçiş dönemi için)
        ...
        # Splitter'a ekle
        splitter.addWidget(self.search_results_widget)
        splitter.addWidget(self.result_widget)
        splitter.setSizes([400, 200])  # Varsayılan boyutlar
        
        # Başlangıçta eski sonuç widget'ını gizle
    ...
    def apply_theme(self, theme_name=None):
        ...
        if theme_name is None:
            theme_name = self.config.get("preferences.theme", "system")
    ...
    def _load_window_position(self):
        ...
        try:
            ...
            if all(v is not None for v in [pos_x, pos_y, width, height]):
                ...
                if (
                    0 <= pos_x <= screen.width() - 100
                    and 0 <= pos_y <= screen.height() - 100
                ):
    ...
    def perform_search(self, query: str, search_type: str):
        ...
        Args:
            ...
            search_type (str): Arama türü (text, semantic, exact, mixed)
        """
        if not query.strip():
            self.status_label.setText("Lütfen arama terimi giriniz")
            return
        ...
        try:
            # Arama türüne göre arama yap
            start_time = time.time()
            if search_type == "text":
                self.logger.info(f"Metin araması yapılıyor: {query}")
                results = self.search_engine.search(query)
            elif search_type == "semantic":
                self.logger.info(f"Semantik arama yapılıyor: {query}")
                results = self.search_engine.semantic_search(query)
            elif search_type == "exact":
                results = self.search_engine.exact_search(query)
            else:
                results = self.search_engine.search(query)  # Varsayılan

            self.last_search_results = results
            search_duration = time.time() - start_time
            self.logger.info(f"Arama tamamlandı. {len(results)} sonuç bulundu. Süre: {search_duration:.2f}s")

            if not results:
                self.status_label.setText("Sonuç bulunamadı")
                return

            # Arama sonuçlarını yeni widget'a aktar
            result_dicts = []
            for result in results:
                try:
                    result_dict = {
                        'id': result.id,
                        'title': result.title or 'Başlıksız',
                        'content': result.content or '',
                        'document_title': result.document_title or 'Belgesiz',
                        'document_type': result.document_type or 'Bilinmeyen',
                        'score': float(getattr(result, 'score', 0.0)),
                        'match_type': getattr(result, 'match_type', 'unknown'),
                        'is_repealed': bool(getattr(result, 'is_repealed', False)),
                        'is_amended': bool(getattr(result, 'is_amended', False)),
                        'article_number': getattr(result, 'article_number', None),
                        'law_number': getattr(result, 'law_number', None),
                        'document_id': getattr(result, 'document_id', None),
                        'snippet': getattr(result, 'snippet', ''),
                        'relevance': float(getattr(result, 'relevance', 0.0))
                    }
                    result_dicts.append(result_dict)
                except Exception as e:
                    self.logger.error(f"Sonuç işlenirken hata: {e}", exc_info=True)
            
            # Arama sonuçlarını widget'a aktar
            self.search_results_widget.set_search_results(query, result_dicts)
            
            # İlk sonucu otomatik seç ve önizle
            if result_dicts:
                self.search_results_widget.select_first_result()
            
            # Eski sonuç widget'ını güncelle (geçiş dönemi için)
            self.result_widget.display_results(results)
            
            self.status_label.setText(f"{len(results)} sonuç bulundu ({search_duration:.2f}s)")
            
        except Exception as e:
            error_msg = str(e) or "Bilinmeyen hata"
            self.logger.error(f"Arama hatası: {error_msg}", exc_info=True)
            self.status_label.setText(f"Arama hatası: {error_msg}")
            
            # Kullanıcıya hata mesajını göster
            QMessageBox.critical(
                self,
                "Arama Hatası",
                f"Arama sırasında bir hata oluştu:\n\n{error_msg}\n\n"
    ...
    def display_article_detail(self, result: SearchResult):
        ...
        content += "</p>"
        content += (
            f"<p><strong>Skor:</strong> {result.score:.3f} ({result.match_type})</p>"
        )
    ...
    def on_preview_document_selected(self, document_data: dict):
        ...
        try:
            # Eğer yeni bir sekmede açma isteniyorsa
            open_in_new_tab = document_data.pop('new_tab', False)
            
            # Belgeyi yükle
            doc_id = document_data.get('id')
            if not doc_id:
                self.logger.warning("Belge ID'si bulunamadı")
                return

            # Veritabanından belge bilgilerini al
            doc = self.db.get_document(doc_id)
            if not doc:
                self.logger.warning(f"{doc_id} ID'li belge bulunamadı")
                return
                
            # Eğer belge sözlük değilse, sözlüğe çevir
            ...
            # Yeni sekmede açma seçeneği aktifse
            if open_in_new_tab and hasattr(self, 'tab_widget'):
                # Yeni bir sekme oluştur ve belgeyi yükle
                from .document_viewer import DocumentViewer
                viewer = DocumentViewer()
                
                # Sinyalleri bağla
                viewer.text_selected.connect(self.on_preview_text_selected)
                
                # Belgeyi yükle
                viewer.load_document(doc)
                
                # Sekmeye ekle
                tab_index = self.tab_widget.addTab(viewer, doc.get('title', 'Yeni Sekme')[:20] + '...')
                self.tab_widget.setCurrentIndex(tab_index)
                
                # Eğer belge dosya yoluna sahipse, dosyayı da yükle
                if doc.get('file_path') and os.path.exists(doc['file_path']):
                    self.update_document_info_for_viewer(doc['file_path'])
            else:
                # Normal şekilde yükle
                self.load_document_in_viewer(doc)

        except Exception as e:
            self.logger.error(f"Belge yükleme hatası: {str(e)}", exc_info=True)
            QMessageBox.critical(self, "Hata", f"Belge yüklenirken hata oluştu: {str(e)}")
    ...
    def update_detail_panel(self, document_data: dict):
        ...
        try:
            ...
            self.logger.error(f"Detay paneli güncelleme hatası: {e}")
            self.detail_content.setPlainText("Detaylar yüklenirken bir hata oluştu.")
    ...
    def load_notes_for_document(self, document_id: int):
        ...
        try:
            ...
            if notes:
                notes_text = "\n\n".join(
                    [f"[{note[1][:10]}] {note[0]}" for note in notes]
                )
    ...
    def on_filter_changed(self):
        ...
        if hasattr(self.search_widget, "last_query") and self.search_widget.last_query:
            self.perform_search(
                self.search_widget.last_query, self.search_widget.last_search_type
            )
            
    def on_document_selected(self, document_data: dict):
        """Belge ağacından belge seçildiğinde çağrılır"""
        try:
            ...
            # Eğer detay paneli gizliyse göster
            if hasattr(self, 'detail_panel') and self.detail_panel.isHidden():
                self.detail_panel.show()
                
        except Exception as e:
            self.logger.error(f"Belge yükleme hatası: {e}", exc_info=True)
            QMessageBox.critical(self, "Hata", f"Belge yüklenirken hata oluştu: {e}")
    ...
    def select_and_process_files(self):
        ...
        try:
            ...
            for i, file_path in enumerate(files):
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        failed_files.append(
                            f"{Path(file_path).name}: {result.get('error', 'Bilinmeyen hata')}"
                        )
                        self.logger.error(
                            f"Dosya ekleme başarısız: {file_path} - {result.get('error')}"
                        )
            ...
            self.status_bar.showMessage(
                f"Belge ekleme tamamlandı: {processed_count} başarılı", 5000
            )
            ...
            QMessageBox.critical(
                self, "Hata", f"Dosya seçimi sırasında hata oluştu:\n{e}"
            )
    ...
    def rebuild_semantic_index(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                if success:
                    QMessageBox.information(
                        self, "Başarılı", "Semantik indeks yeniden oluşturuldu"
                    )
                    self.status_bar.showMessage("İndeks yenileme tamamlandı", 3000)
                else:
                    QMessageBox.warning(
                        self, "Uyarı", "İndeks yenileme sırasında sorunlar oluştu"
                    )
    ...
    def vacuum_database(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                self.db.vacuum()
                QMessageBox.information(
                    self, "Başarılı", "Veritabanı bakımı tamamlandı"
                )
        ...
        try:
            ...
            if self.file_watcher:
                ...
                self.file_watcher_status.update_status(
                    status["is_watching"], status["queue_size"]
                )
    ...
    def add_to_favorites(self):
        ...
        try:
            ...
            if result:
                ...
                if cursor.fetchone():
                    QMessageBox.information(
                        self, "Bilgi", "Bu madde zaten favorilerinizde"
                    )
                    cursor.close()
                    return
    ...
    def dragEnterEvent(self, event: QDragEnterEvent):
        ...
        if event.mimeData().hasUrls():
            ...
            if valid_files:
                ...
                # Visual feedback
                self.status_bar.showMessage(
                    f"{len(valid_files)} desteklenen dosya algılandı"
                )
            else:
                event.ignore()
        ...
        try:
            ...
            if valid_files:
                ...
                reply = QMessageBox.question(
                    ...
                    + (
                        f"\n... ve {len(valid_files)-5} dosya daha"
            ...
            QMessageBox.critical(
                self, "Hata", f"Dosya ekleme sırasında hata oluştu:\n{e}"
            )
    ...
    def on_preview_text_selected(self, selected_text: str):
        ...
        try:
            ...
            self.status_bar.showMessage(
                f"Metin kopyalandı: {len(selected_text)} karakter"
            )
    ...
    def update_document_info_for_viewer(self, file_path: str):
        ...
        try:
            ...
            self.detail_title_label.setText(
                f"PDF Görüntüleyici - {Path(file_path).name}"
            )
    ...
    def load_document_in_viewer(self, document_data: dict):
        ...
        try:
            ...
            self.logger.info(
                f"Belge viewer'da yüklendi: {document_data.get('title', 'Bilinmeyen')}"
            )
    ...
    def process_dropped_files(self, file_paths: List[str]):
        ...
        try:
            ...
            for i, file_path in enumerate(file_paths):
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        self.logger.error(
                            f"Dosya ekleme başarısız (D&D): {file_path} - {result.get('error')}"
                        )

                except Exception as e:
                    failed_files.append(f"{Path(file_path).name}: {str(e)}")
                    self.logger.error(
                        f"Dosya işleme exception (D&D): {file_path} - {e}"
                    )
            ...
            self.stats_widget.refresh_stats()
            self.status_bar.showMessage(
                f"Drag & Drop tamamlandı: {processed_count} başarılı", 5000
            )
            ...
            QMessageBox.critical(
                self, "Hata", f"Dosya işleme sırasında hata oluştu:\n{e}"
            )
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            if hasattr(self, "last_search_results") and self.last_search_results:
                self.config.set(
                    "search.last_results_count", len(self.last_search_results)
                )
    ...
    def _save_window_position(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Pencere pozisyonu kaydedildi: {pos.x()}, {pos.y()}, {size.width()}x{size.height()}"
            )
