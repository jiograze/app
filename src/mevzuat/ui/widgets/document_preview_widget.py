...
class DocumentPreviewWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.current_highlight_format.setBackground(
    ...
    def load_document(self, document_data: Dict[str, Any]):
        ...
        try:
            self.logger.info(
                f"Belge yükleniyor: {document_data.get('title', 'Bilinmeyen')}"
            )
    ...
    def highlight_all_results(self):
        ...
        for result in self.search_results:
            self.highlight_text_range(
                result["start"], result["end"], self.highlight_format
            )
    ...
    def on_search_result_clicked(self, item: QListWidgetItem):
        ...
        for i, result in enumerate(self.search_results):
            if (
                result["start"] == result_data["start"]
                and result["end"] == result_data["end"]
            ):
