...
from mevzuat.utils.logger import TimedOperation, log_performance_metric
from .semantic_search_alternative import SimpleTfIdfSemanticSearch
from .text_analyzer import EnhancedFTSQueryBuilder, TurkishTextAnalyzer
...
class SearchEngine:
    ...
    def __init__(self, config_manager, database_manager):
        ...
        # Performans ayarları
        self.use_threading = config_manager.get(
            "performance.threading.use_process_pool", False
        )
        self.max_workers = config_manager.get(
        ...
        if self.use_tfidf_fallback:
            try:
                ...
                if self.tfidf_search.initialize(documents):
                    self.logger.info(
                        f"TF-IDF semantik arama başlatıldı: {len(documents)} dokuman"
                    )
                else:
    ...
    def _load_faiss_index(self):
        ...
        try:
            ...
            if index_path.exists() and map_path.exists():
                ...
                self.logger.info(
                    f"FAISS indeksi yüklendi: {self.faiss_index.ntotal} vektör"
                )
            else:
                # Yeni indeks oluştur
    ...
    def search(
        ...
        try:
            ...
            cache_key = self._generate_cache_key(
                query, document_types, search_type, include_repealed
            )
            ...
            self.db.add_search_to_history(
                query, search_type, len(results), execution_time
            )
            ...
            log_performance_metric(
                "search_total",
                execution_time,
                {"query": query[:50], "type": search_type, "results": len(results)},
            )

            self.logger.info(
                f"Arama tamamlandı: '{query}' -> {len(results)} sonuç ({execution_time:.1f}ms)"
            )
            return results
    ...
    def _semantic_search(
        ...
        with TimedOperation("semantic_search", details={"query": query[:50]}):
            ...
            if self.use_tfidf_fallback:
                try:
                    tfidf_results = self.tfidf_search.search(
                        query, top_k=self.max_results * 2
                    )
                    ...
                    for article_id, score in tfidf_results:
                        ...
                        if (
                            document_types
                            and article_data["document_type"] not in document_types
                        ):
                            continue
    ...
    def _mixed_search(
        ...
        if self.use_threading:
            ...
            return self._mixed_search_sequential(
                query, document_types, include_repealed
            )

    def _mixed_search_sequential(
        self,
        query: str,
        document_types: List[str] = None,
        include_repealed: bool = False,
    ...
    def _mixed_search_threaded(
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            future_keyword = executor.submit(
                self._keyword_search, query, document_types, include_repealed
            )
            future_semantic = executor.submit(
                self._semantic_search, query, document_types, include_repealed
            )
    ...
    def _combine_search_results(
        self, keyword_results: List[SearchResult], semantic_results: List[SearchResult]
    ) -> List[SearchResult]:
    ...
    def _generate_semantic_highlights(
        self, content: str, query: str, max_highlights: int = 2
    ) -> List[str]:
    ...
    def rebuild_index(self):
        ...
        try:
            ...
            cursor = self.db.connection.cursor()
            cursor.execute(
                "SELECT id, content_clean FROM articles WHERE content_clean IS NOT NULL"
            )
            ...
            self.logger.info(
                f"FAISS indeksi yeniden oluşturuldu: {len(articles)} artikel"
            )
            return True
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "semantic_enabled": self.semantic_enabled,
                "cache_size": (
                    len(self.search_cache) if hasattr(self, "search_cache") else 0
                ),
            }
    ...
    def rebuild_index(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Semantik indeks oluşturuldu: {len(articles)} makale indekslendi"
            )
            return True
