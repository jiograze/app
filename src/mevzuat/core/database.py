...
class DatabaseManager:
    ...
    def initialize(self):
        """Veritabanını başlat, tabloları oluştur ve gerekli migrasyonları çalıştır"""
        try:
    ...
    def _run_migrations(self):
        ...
        # Migrasyonların sıralı çalışması için sıralı import
        from .db_migrations import (
            migration_0001_add_missing_tables,
            # Yeni migrasyonlar buraya eklenecek
        )

        # Migrasyonların sırasıyla çalıştırılması
        migrations = [
            ('0001_add_missing_tables', migration_0001_add_missing_tables.migrate),
            # Yeni migrasyonlar buraya eklenecek
        ]

        # Migrasyon tablosunu oluştur (eğer yoksa)
        ...
        for name, migrate_func in migrations:
            if name not in applied_migrations:
                try:
                    ...
                    self.logger.info(f"Migrasyon uygulandı: {name}")

                except Exception as e:
                    self.connection.rollback()
                    self.logger.error(f"Migrasyon hatası ({name}): {e}")
                    raise
    ...
    def search_articles(
        self, query: str, document_types: List[str] = None, limit: int = 20
    ) -> List[Dict[str, Any]]:
        """FTS ile madde arama"""
    ...
    def add_search_to_history(
        self, query: str, query_type: str, results_count: int, execution_time_ms: float
    ):
