...
class MevzuatFileHandler(FileSystemEventHandler):
            ...
            try:
                if self._wait_for_file_stable(file_path):
                    ...
                    self.ingest_queue.put(
                        {"path": file_path, "hash": file_hash, "timestamp": time.time()}
                    )
    ...
    def _wait_for_file_stable(self, file_path: str, max_wait: int = 10) -> bool:
        ...
        while (
            stable_count < 3 and wait_count < max_wait
        ):  # 3 kez aynı boyutta ise kararlı
            try:
...
class FileWatcher:
    ...
    def __init__(self, config_manager, document_processor):
        ...
        self.health_check_interval = (
            config_manager.get("watch_health_check_min", 10) * 60
        )
    ...
    def start_watching(self):
        ...
        try:
            ...
            self.observer.schedule(
                self.file_handler, str(self.watch_folder), recursive=True
            )
    ...
    def _initial_scan(self):
        ...
        try:
            ...
            for file_path in self.watch_folder.rglob("*"):
                if file_path.is_file() and self.file_handler._is_supported_file(
                    str(file_path)
                ):
    ...
    def _process_file(self, file_info: dict) -> bool:
        ...
        try:
            ...
            if result["success"]:
                ...
                self.logger.error(
                    f"Dosya işleme başarısız: {file_path} - {result.get('error', 'Bilinmeyen hata')}"
                )

                # Başarısız ise karantinaya taşı
                self._move_to_quarantine(
                    file_path, result.get("error", "Bilinmeyen hata")
                )
                return False
    ...
    def _move_processed_file(self, file_path: str, result: dict):
        ...
        try:
            ...
            if "organization" in result and result["organization"].get("success"):
                ...
                self.logger.info(
                    f"  Klasör yapısı: {org_info.get('organized_structure', 'N/A')}"
                )
            else:
            ...
            try:
                ...
                if self.stats["last_activity"]:
                    ...
                    if time_since_activity > self.health_check_interval:
                        self.logger.warning(
                            f"Son {self.health_check_interval/60:.1f} dakikada dosya aktivitesi yok"
                        )
