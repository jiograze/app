...
class ConfigManager:
    ...
    def _replace_placeholders(self):
        ...
        self.config_data = self._recursive_replace_placeholders(
            self.config_data, "{MEVZUAT_FOLDER}", str(base_folder)
        )
    ...
    def _recursive_replace_placeholders(
        self, obj: Any, placeholder: str, value: str
    ) -> Any:
        """Nesnede placeholder'ları özyinelemeli olarak değiştir"""
        if isinstance(obj, dict):
            return {
                k: self._recursive_replace_placeholders(v, placeholder, value)
                for k, v in obj.items()
            }
        elif isinstance(obj, list):
            return [
                self._recursive_replace_placeholders(item, placeholder, value)
                for item in obj
            ]
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "logging": {"level": "INFO", "rotate_size_mb": 5, "keep_files": 5},
            "embedding": {
    ...
    def save(self, data: Optional[Dict] = None) -> bool:
        ...
        try:
            ...
            # Veriyi al
            save_data = self.config_data if data is None else data

            # Yedek al
            if self.config_path.exists():
                backup_path = self.config_path.with_suffix(f".{int(datetime.now().timestamp())}.bak")
    ...
    def save_config(self, data: Optional[Dict] = None) -> bool:
        """save_config is an alias for save method for backward compatibility"""
