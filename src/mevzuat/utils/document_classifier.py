...
# BERT tabanlı başlık analizi için
...
class DocumentClassifier:
    ...
    def __init__(self, config_manager):
        ...
        # BERT tabanlı başlık analizörünü yükle
        try:
            self.title_analyzer = BERTTitleAnalyzer()
            self.logger.info("BERT tabanlı başlık analizörü başarıyla yüklendi")
        except Exception as e:
            self.logger.error(f"Başlık analizörü yüklenirken hata oluştu: {str(e)}")
            self.title_analyzer = None
        ...
        self.document_type_patterns = {
            ...
            "TÜZÜK": [r"(?i)tüzük", r"(?i)tüzüğü$"],
            "YÖNETMELİK": [r"(?i)yönetmelik", r"(?i)yönetmeliği$"],
            "YÖNERGE": [r"(?i)yönerge", r"(?i)genelge"],
            "TEBLİĞ": [r"(?i)tebliğ"],
            "KARAR": [
    ...
    def classify_document(
        self, text: str, filename: str, metadata: Dict = None
    ) -> Dict[str, Any]:
        """
        Belgeyi sınıflandır
        ...
        try:
            ...
            # Başlık çıkarma
            title_result = self.extract_title(text, filename)
            result["title"] = title_result["title"]
            result["confidence"] = title_result["confidence"]

            # Belge türü tespiti
            doc_type, confidence = self._classify_document_type(
                text, 
                result["title"], 
                filename
            )
            ...
            if doc_type in ["KANUN", "KHK", "YÖNERGE", "YÖNETMELİK"]:
                law_number = self._extract_law_number(text, result["title"], filename)

            result["law_number"] = law_number

            # Kategori belirleme
            category, subcategory = self._determine_category(text, result["title"], doc_type)
            ...
            self.logger.info(f"Belge sınıflandırıldı: {doc_type} - {result['title']}")
            return result

        except Exception as e:
            self.logger.error(f"Belge sınıflandırma hatası: {e}")
            return {"document_type": "DIGER", "title": filename, "confidence": 0.0}

    def extract_title(self, text: str, filename: str = "") -> Dict[str, Any]:
        """
        Metinden başlık çıkarır.
        ...
        Returns:
            Dict: Başlık ve güven skoru içeren sözlük
        """
        # Eğer BERT analizörü yüklenemediyse veya metin çok kısaysa basit yöntem kullan
        if not self.title_analyzer or len(text) < 20:
            first_line = text.split("\n")[0].strip()
            return {
                "title": first_line if first_line else "Belge Başlığı Bulunamadı",
                "confidence": 0.5
            }
        
        try:
            # BERT ile başlık çıkar
            title = self.title_analyzer.extract_title_from_text(text)
            
            if title:
                # Başlık bulundu, güven skorunu hesapla
                _, confidence = self.title_analyzer.is_title(title)
                return {
                    "title": title,
                    "confidence": confidence
                }
            else:
                # Başlık bulunamadı, ilk satırı kullan
                first_line = text.split("\n")[0].strip()
                return {
                    "title": first_line if first_line else "Belge Başlığı Bulunamadı",
                    "confidence": 0.3
            ...
            self.logger.error(f"Başlık çıkarılırken hata oluştu: {str(e)}")
            first_line = text.split("\n")[0].strip()
            return {
                "title": first_line if first_line else "Belge Başlığı Bulunamadı",
                "confidence": 0.2
    ...
    def _extract_law_number(
        self, text: str, title: str, filename: str = ""
    ) -> Optional[str]:
    ...
    def _determine_category(self, text: str, title: str, doc_type: str) -> tuple:
        ...
        category_keywords = {
            ...
            "SAĞLIK": ["sağlık", "tıp", "hastane", "doktor", "eczane", "ilaç"],
            "EĞİTİM": [
                "eğitim",
                "öğretim",
                "okul",
            ...
            "ÇEVRE": ["çevre", "orman", "su", "hava", "kirlilik", "atık"],
        }
    ...
    def _extract_publication_date(
        ...
        if metadata:
            if "pdf_metadata" in metadata and metadata["pdf_metadata"].get(
                "CreationDate"
            ):
                return metadata["pdf_metadata"]["CreationDate"]
            if "core_properties" in metadata and metadata["core_properties"].get(
                "created"
            ):
    ...
    def get_classification_stats(self) -> Dict[str, Any]:
        ...
        return {"total_classified": 0, "by_type": {}, "accuracy": 0.0}
