...
def create_test_documents():
    ...
    txt_file2 = tempfile.NamedTemporaryFile(
        mode="w", suffix=".txt", delete=False, encoding="utf-8"
    )
    ...
    try:
        ...
        for i, file_path in enumerate(test_files, 1):
            ...
            if result["success"]:
                ...
                print(
                    f"  🏷️ Belge türü: {result['classification'].get('document_type', 'N/A')}"
                )
            else:
...
def main():
    ...
    print("🎯 GENEL SONUÇ:")
    print(
        f"   Document Processing: {'✅ BAŞARILI' if test1_success else '❌ BAŞARISIZ'}"
    )
    print(
        f"   Duplicate Detection: {'✅ BAŞARILI' if test2_success else '❌ BAŞARISIZ'}"
    )
