    ...
    try:
        ...
        if 'articles' in tables:
            ...
            if 'content_clean' in columns:
                ...
                cursor.execute("SELECT COUNT(*) FROM articles WHERE content_clean IS NOT NULL AND content_clean != ''")
                clean_count = cursor.fetchone()[0]
                print(f"📊 Temiz içerik sayısı: {clean_count}")
                
                # Örnek temiz içerik
                cursor.execute("SELECT id, content_clean FROM articles WHERE content_clean IS NOT NULL AND content_clean != '' LIMIT 1")
